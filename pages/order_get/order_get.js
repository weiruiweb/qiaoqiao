...
Page({
		...
		const callback = (res) => {
			if (res.info.data.length > 0) {
				self.data.userData = res.info.data[0],
					self.data.submitData.name = self.data.userData.info.name,
					self.data.submitData.phone = self.data.userData.info.phone
			};
			self.setData({
				web_submitData: self.data.submitData,
	...
	pay(order_id) {
		...
		if (self.data.userData && self.data.userData.info.level == 0) {
			if (wx.getStorageSync('info').thirdApp.custom_rule[0] && wx.getStorageSync('info').thirdApp.custom_rule[0].getScoreRatio &&
				wx.getStorageSync('info').thirdApp.custom_rule[0].getScoreRatio > 0) {
				postData.payAfter.push({
					tableName: 'FlowLog',
					FuncName: 'add',
					data: {
		...
		if (self.data.distributionData && self.data.distributionData.userInfo[0] && self.data.distributionData.userInfo[0].level &&
			self.data.distributionData.userInfo[0].level == 1) {
			postData.payAfter.push({
				tableName: 'FlowLog',
				FuncName: 'add',
				data: {
		...
		if (self.data.distributionData && self.data.distributionData.userInfo[0] && self.data.distributionData.userInfo[0].level &&
			self.data.distributionData.userInfo[0].level == 2) {
			postData.payAfter.push({
				tableName: 'FlowLog',
				FuncName: 'add',
				data: {
